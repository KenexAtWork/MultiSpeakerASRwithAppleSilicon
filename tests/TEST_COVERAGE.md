...
| `test_srt_parser.py` | 單元 | 12 | ✅ 已完成 |
| `test_error_handling.py` | 整合 | 8 (3 跳過) | ✅ 已完成 |
...
### 4. SRT 解析邏輯 (`gui/ui/main_window.py` - `_parse_srt`)
...
**未覆蓋功能：**
- ⚠️ 超大 SRT 檔案（>10000 段落）
- ⚠️ 不同編碼格式（UTF-16, GBK）
- ⚠️ 時間戳倒退情況

**測試檔案：** `test_srt_parser.py`
...
### 高優先級（影響核心功能）
1. ~~**SRT 解析邏輯測試**~~ ✅ 已完成 - 驗證字幕顯示正確
2. ~~**錯誤處理測試**~~ ✅ 已完成 - 提升使用者體驗
3. **GUI ASR Worker 測試** - 確保 subprocess 執行穩定
...
- 2026-02-23: 新增 SRT 解析測試（12 tests）和錯誤處理測試（8 tests），覆蓋率提升至 54%
